 ...
 */
export function createJsonTransformStream(writableStrategy, readableStrategy) {
    ...
    }, resolveStrategy(writableStrategy), resolveStrategy(readableStrategy));
}
 ...
 */
export function createTimestampTransformStream(maxAgeMs = 0, writableStrategy, readableStrategy) {
    return new TransformStream({
        transform(chunk, controller) {
            const now = Date.now();
            const isStale = maxAgeMs > 0 && now - chunk.timestamp > maxAgeMs;
            controller.enqueue(isStale || !chunk.timestamp ? { ...chunk, timestamp: now } : chunk);
        },
    }, resolveStrategy(writableStrategy), resolveStrategy(readableStrategy));
}
// ── Filter Transform ──────────────────────────────────────────────────────────
 ...
 */
export function createFilterTransformStream(predicate, writableStrategy, readableStrategy) {
    return new TransformStream({
        transform(chunk, controller) {
            if (predicate(chunk)) {
 ...
 */
export function createMapTransformStream(mapper, writableStrategy, readableStrategy) {
    return new TransformStream({
        transform(chunk, controller) {
 ...
 */
export function createMessageEncoderStream(writableStrategy, readableStrategy) {
